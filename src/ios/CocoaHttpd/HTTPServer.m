...
- (id)init
{
	if ((self = [super init]))
	{
        ...
        // Register for notifications of websocket broadcast
		[[NSNotificationCenter defaultCenter] addObserver:self
		                                         selector:@selector(webSocketBroadcast:)
		                                             name:WebSocketBroadcastNotification
		                                           object:nil];
...
- (void)broadcastMessage:(NSString *)msg fromWebsocket:(WebSocket *)ws
{
    for (WebSocket *webSocket in webSockets)
    {
        if (webSocket!=ws) [webSocket sendMessage:msg];
    }
}
...
- (void)webSocketBroadcast:(NSNotification *)notification
{
	[self broadcastMessage:[[notification userInfo] objectForKey:@"message"] fromWebsocket:[notification object]];
}
